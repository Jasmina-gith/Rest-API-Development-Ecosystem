...
router.post('/register', async (req,res) => {
    ...
    bcrypt.hash(req.body.password, 10, (err, password) => {
        ...
        .then(async result => {
            ...
            const data = { userId: user.user_id, username: user.username, role: user.role || 'USER' }
...
router.post('/login', async (req,res) => {
    ...
    const { data, error } = await Pool.from('Users').select('user_id, username, password, role').eq('username', username);
    if (error) {
...
router.post('/refresh', (req,res) => {
    ...
    .then((result) => {
        ...
        jwt.verify(token, process.env.REFRESH_KEY as string, async (err: any, result: any) => {
            ...
            const { data: userData, error: userError } = await Pool.from('Users').select('role').eq('user_id', userId).single()
            if (userError) {
                return res.status(500).json({ error: userError.message })
            }

            const role = userData?.role || 'USER'
            const accessToken = getAccessToken({ userId, username, role })
            const refreshToken = await getRefreshToken({ userId, username, role })
            res.status(201).json({ accessToken, refreshToken })
        })
    })
})
